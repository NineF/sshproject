...
public class DateUtil {
	...
	private static int timeCheckSum;
	...
	/*
	 * 对时间格式进行解析并还原原来的时间格式 对数据进行还原 仅限于debug使用
	 */
	...
	/*
	 * 将byte转化为字符串 将有符号byte转化为无符号数字 debug使用
	 */
	...
	/*
	 * 将数组封装成帧 每一个数据帧由以下几个部分组成 1)数据包头部 head 0X2F 2)数据包命令 CMD 0X5A 3)数据个数 length
	 * of data 7 4)校验和 H8/L8 byte of check sum(高字节在前 低字节在后) 5)数据结尾标志 tail OX30
	 * 6)可采用线程进行获取当前时间
	 */
