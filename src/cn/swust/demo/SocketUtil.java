...
public class SocketUtil {
	/*
	 * 类方法 不可改变 不接受继承 扫描获取可用的串口 将可用串口添加至list并保存至list
	 */
	...
	/*
	 * 串口常见设置 1)打开串口 2)设置波特率 根据单板机的需求可以设置为57600 ... 3)判断端口设备是否为串口设备 4)端口是否占用
	 * 5)对以上条件进行check以后返回一个串口设置对象new UARTParameterSetup()
	 * 6)return:返回一个SerialPort一个实例对象，若判定该com口是串口则进行参数配置 若不是则返回SerialPort对象为null
	 */
